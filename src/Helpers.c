...
int HL_get_wraps ( int beg , int end )
{
    int k = 0;
    int out = 0;
    for ( k = beg ; k < end ; ++ k ) {
...
int		HL_Get_current_word ( char **text , int *pos_out )
{
    ...
    if ( 1 ) {
        ...
        delta = SendMessage ( hwndEdit , SCI_GETTEXTRANGE , 0 , ( LPARAM ) &tr ) ;
        *text = tr.lpstrText;
        if ( pos_out ) {
...
VOID HL_Highlight_turn ( BOOL edit_sel )
{
...
VOID HL_Edit_selection_start()
{
    ...
    if ( b_HL_edit_selection ) {
        ...
        int beg = SendMessage ( hwndEdit , SCI_WORDSTARTPOSITION , pos , 1 );
        int end = SendMessage ( hwndEdit , SCI_WORDENDPOSITION , pos , 1 );
        SendMessage ( hwndEdit , SCI_BEGINUNDOACTION , 0, 0 );
        SendMessage ( hwndEdit , SCI_SETSEL , beg , end );
    }
}


VOID HL_Edit_process_changes ( BOOL rollback )
{
    struct	Sci_TextRange	tr;
    int	wlen = strlen ( _hl_sel_edit_prev );
    int	srchlen = 0;
    int k;
    char	*nword = 0;
    int	cpos = 0;
    ...
    if ( nword ) {
        ...
        for ( k = 0 ; k < _hl_sel_len; ++k ) {
            ...
            if ( replace ) {
                ...
                if ( tr.chrg.cpMin == _hl_sel_edit_word_pos && !rollback ) { // at word
