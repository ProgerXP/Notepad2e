...
BOOL TextRange_Init(const StringSource* pSS, const RecodingAlgorithm* pRA, struct TTextRange* pTR)
{
  ...
  pTR->m_originalSelection = TRUE;
  pTR->m_emptyOriginalSelection = (pTR->m_iSelStart == pTR->m_iSelEnd);
  if (pRA->recodingType == ERT_CALW)
  {
    if (pTR->m_emptyOriginalSelection)
      pTR->m_iSelStart = StringSource_GetLineStart(pSS, pTR->m_iSelStart);
    pTR->m_iSelEnd = min(pTR->m_iSelStart + DEFAULT_RECODING_BUFFER_SIZE, pTR->m_emptyOriginalSelection ? StringSource_GetLength(pSS) : pTR->m_iSelEnd);
    ...
    if (!pTR->m_originalSelection)
    {
      if (pTR->m_iSelEnd >= pTR->m_iSelStart + DEFAULT_RECODING_BUFFER_SIZE)
      {
        ...
        SciCall_SetCurrentPos(pTR->m_iSelEnd);
        SciCall_AltWordLeft2();
        pTR->m_iSelEnd = SciCall_GetCurrentPos();
    ...
    {
        // #NOTE: move selection to previous line end to prevent trailing EOL in buffer
        const auto line = SciCall_LineFromPosition(pTR->m_iSelEnd);
        pTR->m_iSelEnd = SciCall_LineEndPosition(line - 1);
    }
  }
  else if (pTR->m_emptyOriginalSelection)
  {
    pTR->m_iSelStart = 0;
...
BOOL TextRange_GetNextDataPortion(StringSource* pSS, const RecodingAlgorithm* pRA, struct TTextRange* pTR, struct TTextBuffer* pTB)
{
  const long iEnd = min(pTR->m_iPositionCurrent + pTB->m_iSize - 1, pTR->m_iSelEnd);
  if (StringSource_GetText(pSS, pRA, pTB->m_ptr, pTR->m_iPositionCurrent, iEnd))
  {
...
void Recode_Run(RecodingAlgorithm* pRA, StringSource* pSS, const int bufferSize)
{
  ...
  for (int i = 0; i < pRA->iPassCount; ++i)
  {
    ...
    while (StringSource_IsDataPortionAvailable(pSS, pRA, &ed) && !RecodingAlgorithm_ShouldBreak(pRA))
    {
      if (ed.m_tb.m_iSize < iRecodingBufferSize)
      {
        TextBuffer_Init(&ed.m_tb, iRecodingBufferSize);
      }
      if (TextRange_GetNextDataPortion(pSS, pRA, &ed.m_tr, &ed.m_tb))
      {
...
BOOL Recode_ProcessDataPortion(RecodingAlgorithm* pRA, StringSource* pSS, EncodingData* pED)
{
  ...
  if (pED->m_bIsEncoding)
  {
    ...
    while (TextBuffer_IsDataPortionAvailable(&pED->m_tb, pRA->iRequiredCharsForEncode)
           ...
           && !RecodingAlgorithm_ShouldBreak(pRA))
    {
  ...
  if (charsProcessed)
  {
    ...
    if (pSS->hwnd)
    {
      ...
      else
      {
          ...
          || pRA->iResultEnd)
        {
          if (pRA->iResultEnd)
          {
            SciCall_SetTargetStart(pRA->iResultStart);
            SciCall_SetTargetEnd(pRA->iResultEnd);
            pRA->iResultSelEnd = pRA->iResultStart + pED->m_tbRes.m_iPos;
          }
          else if (RecodingAlgorithm_IsRestrictedForHWNDUse(pRA))
          {
            SciCall_SetTargetStart(pED->m_tr.m_iSelStart);
            SciCall_SetTargetEnd(pED->m_tr.m_iSelEndOriginal);
          }
          else
          {
    ...
    if (iCursorOffset != 0)
    {
      ...
      pED->m_tr.m_iSelOffset += iCursorOffset;
    }
  }
