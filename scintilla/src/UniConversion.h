...
unsigned int UTF8Length(const wchar_t *uptr, unsigned int tlen, const bool processNULL = false); // [n2e]: #145 Allow NULL character substitution when using regexp-replace (\x00)
void UTF8FromUTF16(const wchar_t *uptr, unsigned int tlen, char *putf, unsigned int len, const bool processNULL = false); // [n2e]: #145 Allow NULL character substitution when using regexp-replace (\x00)
